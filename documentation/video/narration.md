# Narration direction

- Voice: `en-US-AndrewMultilingualNeural`
- Delivery: neutral, calm, confident
- Rate: 5% slower than the service default
- Captions: generated from the voice service's word timing, then shifted to the
  assembled scene timeline
- Privacy: only the public narration in `segments.json` is sent to the speech
  service; no recording, repository file, or personal content is uploaded

The exact spoken copy lives in `segments.json`, next to its scene start and
duration. The master intentionally uses a synthetic workflow and labels
deterministic product footage on screen.
